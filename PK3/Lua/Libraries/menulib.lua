...
if menulib and menulib.version >= VERSION then return end
...
local function getKeys(p)
	...
	if cmd.forwardmove > 0 or cmd.buttons & BT_WEAPONPREV then
		up = true
	elseif cmd.forwardmove < 0 or cmd.buttons & BT_WEAPONNEXT then
		down = true
	end
...
function menulib.open(p, id, mod, choice)
	...
	if menustates.mousex == nil and menu.mouse then
	...
	if menu.open then -- BUG: menu is nil
...
function menulib.close(p, force)
	...
	if menu.close and not force and menu.close(p, menustate, menu) then return end

	-- Restore the thrust factor
	p.thrustfactor = menustates.thrustfactor

	if not (menustates.pflags & PF_FORCESTRAFE) then
		-- Restore the strafe flag
		p.pflags = $ & ~PF_FORCESTRAFE
	end

	if menustates.pflags & PF_ANALOGMODE then
		-- Restore analog mode
		p.pflags = $ | PF_ANALOGMODE
	end
...
function menulib.set(mod, newmenus)
	...
	for f, v in pairs(defaulttemplate) do
		if generictemplate[f] == nil then
			generictemplate[f] = v
		end
	end
	...
	-- and replace them with their default values
	for _, menu in pairs(menus[mod]) do
		...
		for _, f in ipairs({
			...
			if menu[f] == nil then
				if template and template[f] ~= nil then
					menu[f] = template[f]
				elseif generictemplate[f] ~= nil then
...
local function handleVerticalMenu(p, menu, menustate)
	...
	if menu.mouse then
		...
		if abs(mousedx) >= FU / 2 or abs(mousedy) >= FU / 2 then
			-- Move the cursor
			m.mousex = $ - mousedx
			if m.mousex < 0 then
				m.mousex = 0
			elseif m.mousex > 320 * FU then
				m.mousex = 320 * FU
			end
			m.mousey = $ - mousedy
			if m.mousey < 0 then
				m.mousey = 0
			elseif m.mousey > 200 * FU then
				m.mousey = 200 * FU
			end
			...
			if x >= x1 and x <= x2 and y >= y1 and y <= y2 then
				...
				and y <= y1 + (pointedline - 1) * step + 3 then
					local line = menu[pointedline]
					if menustate.choice ~= pointedline
					and (not line.skip or line.skip ~= true and not line.skip(p)) then
	...
	-- Close the menu
	if bt & BT_CUSTOM2 and not (prevbt & BT_CUSTOM2) then
		menulib.close(p)
	...
	--or bt & BT_ATTACK and not (prevbt & BT_ATTACK) then
	or bt & BT_CUSTOM1 and not (prevbt & BT_CUSTOM1) then
		if line.condition and not line.condition(p) then
			S_StartSound(nil, sfx_lose, p)
		else
			if type(line.ok) == "string" then
				if line.ok == nil or type(line.ok) ~= "string" then print("handleVerticalMenu: menu id is nil! menustate: id = \""..menustate.id.."\", choice = "..menustate.choice) end
				menulib.open(p, line.ok, 1)
			elseif type(line.ok) == "function" then
				if not line.ok(p, menustate) then
					S_StartSound(nil, sfx_menu1, p)
				end
			end
		end
	...
	elseif left then
		if line.left then
			...
			if ok and not line.left(p, menustate) then
				S_StartSound(nil, sfx_menu1, p)
			end
		end
	...
	elseif right then
		if line.right then
			...
			if ok and not line.right(p, menustate) then
				S_StartSound(nil, sfx_menu1, p)
			end
		end
...
local function handleGridMenu(p, menu, menustate)
	...
	-- Confirm
	elseif (bt & BT_JUMP and not (prevbt & BT_JUMP) /*or bt & BT_ATTACK and not (prevbt & BT_ATTACK)*/ or bt & BT_CUSTOM1 and not (prevbt & BT_CUSTOM1)) then
		if menu.dynamic then
			if menu.choices(p, menustate) ~= 0 then
				if menu.ok then
					menu.ok(p, menustate.choice, menustate)
				end
			end
		elseif element then
			if element.condition and not element.condition(p) then
				S_StartSound(nil, sfx_lose, p)
			else
				if type(element.ok) == "string" then
					menulib.open(p, element.ok, 1)
				elseif type(element.ok) == "function" then
					if not element.ok(p, menustate) then
						S_StartSound(nil, sfx_menu1, p)
					end
				end
			end
...
function menulib.handle(p)
	...
	if p.thrustfactor ~= 0 then
		m.thrustfactor = p.thrustfactor
		p.thrustfactor = 0
	end
	if cmd.angleturn ~= m.prevangleturn and p.mo and p.mo.valid then
		p.mo.angle = $ - (cmd.angleturn - m.prevangleturn) * FU
	end
	if cmd.aiming ~= m.prevaiming then
		p.aiming = $ - (cmd.aiming - m.prevaiming) * FU
	end

	-- Force strafe mode while menu is open
	if not (p.pflags & PF_FORCESTRAFE) then
	...
	if menu.condition and not menu.condition(p, menustate) then
		menulib.back(p, menustate)
		return
	end

	if menu.layout == "grid" then
		handleGridMenu(p, menu, menustate)
	else
		handleVerticalMenu(p, menu, menustate)
	end

	if menu.handleextra then
...
for _, h in ipairs({"JumpSpecial", "AbilitySpecial", "SpinSpecial", "JumpSpinSpecial"}) do
	addHook(h, function(p)
		if p.menu then return true end
	end)
end
...
function menulib.drawBlockBox(v, x1, y1, x2, y2, size, p)
	...
	for y = y1 + p.topoffset * s, y2 + p.topoffset * s, size do
		for x = x1 + p.leftoffset * s, x2 + p.leftoffset * s, size do
			drawScaled(x, y, s, p)
		end
	end
end
...
function menulib.drawBlockBorders(v, x1, y1, x2, y2, size, p)
	...
	for x = x1 + p.leftoffset * s, x2 - size - size + FU + p.leftoffset * s, size do
		drawScaled(x, y1, s, p)
	end

	local x = x2 - size + FU
	for y = y1 + p.topoffset * s, y2 - size - size + FU + p.topoffset * s, size do
		drawScaled(x, y, s, p)
	end

	local y = y2 - size + FU
	for x = x1 + size + p.leftoffset * s, x2 - size + FU + p.leftoffset * s, size do
		drawScaled(x, y, s, p)
	end

	for y = y1 + size + p.topoffset * s, y2 - size + FU + p.topoffset * s, size do
		drawScaled(x1, y, s, p)
	end
end
...
local function drawGridMenu(v, p, menu, choice)
	...
	if menu.drawbox then
		menu.drawbox(v, menu, p)
	else
		drawMenuBox(v, x1, y1, w, h, v.cachePatch(menu.background), menu.backgroundsize, v.cachePatch(menu.border), menu.bordersize, not menu.shadow)
	end

	if menu.drawelements then
		...
		if menu.dynamic then
			for i = 1, choices do
				...
				local f = f
				if choice == i and menu.blinkselectedtext then
			...
			for i, element in ipairs(menu) do
				...
				if choice == i and menu.blinkselectedtext then
					f = $ | (leveltime & 8 and V_GREENMAP or V_ORANGEMAP)
				elseif element.condition and not element.condition(p) then
					f = $ | V_GRAYMAP
				end
	...
	if menu.drawcursor ~= nil and choice <= choices then
...
function menulib.draw(v, p)
	...
	if menu.condition and not menu.condition(p, menustate) then return end
	if menu.layout == "grid" then -- BUG: menu is nil
